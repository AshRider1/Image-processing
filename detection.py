...
def plot_performance_per_snr(model, annotations, ft_models=None):
    ...
    noise_dist = [compute_mmap(model, annotations, lambda img, s=s: add_noise(img, severity=s)) for s in noise_sev]
    blur_dist = [compute_mmap(model, annotations, lambda img, k=k: add_motion_blur(img, kernel_size=k)) for k in blur_sev]
    rain_dist = [compute_mmap(model, annotations, lambda img, i=i: add_rain(img, intensity=i)) for i in rain_sev]

    noise_enh = [compute_mmap(model, annotations, lambda img, s=s: denoise(add_noise(img, severity=s))) for s in noise_sev]
    blur_enh = [compute_mmap(model, annotations, lambda img, k=k: deblur(add_motion_blur(img, kernel_size=k))) for k in blur_sev]
    rain_enh = [compute_mmap(model, annotations, lambda img, i=i: derain(add_rain(img, intensity=i))) for i in rain_sev]
    ...
    if ft_models:
        noise_ft = [compute_mmap(ft_models["noise"], annotations, lambda img, s=s: add_noise(img, severity=s)) for s in noise_sev]
        blur_ft = [compute_mmap(ft_models["motion_blur"], annotations, lambda img, k=k: add_motion_blur(img, kernel_size=k)) for k in blur_sev]
        rain_ft = [compute_mmap(ft_models["rain"], annotations, lambda img, i=i: add_rain(img, intensity=i)) for i in rain_sev]
...
def run():
    ...
    for name, (distort_fn, _) in DISTORTIONS.items():
        ...
        per_class, conf = compute_map(ft_models[name], annotations, distort_fn=distort_fn)
